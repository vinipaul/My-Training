...
public class ManagePaymentMethodTest extends Base {
	...
	public void verify_newsField_Is_Editable_when_Edit_Icon_Clicked() throws IOException {
		...
		managePaymentMethodPage.enterUserName((ExcelUtility.getString(1, 0, constants.Constants.FILEPATH, "login")))
				...
				.clickOnSignInButton().clickOnManagePaymentMethods().clickOnEditIcon();
		...
		managePaymentMethodPage.editPaymentMethods(title, paymentLimit).clickOnUpdateButton();
		assertTrue(managePaymentMethodPage.checkForUpdatesInTheTable(title, paymentLimit),
				"Updation In the Table Failed");
	}
}
