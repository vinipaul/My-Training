	...
	public void selectIcon(String input) {
		...
		for(WebElement dashboardIcon:dashboardIcons) {
			 ...
			 if(dashboardIconText.equals(input)) {
				 List <WebElement> moreInfoLinks=driver.findElements(links);
					for(WebElement moreinfoLink:moreInfoLinks)
					{
						String moreinfoattribute=moreinfoLink.getAttribute("href");
						List<WebElement> linksList=driver.findElements(By.xpath("dashboardOptions//following::a[text()='More info ']"));
						if(moreinfoattribute.contains(dashboardIconText)) {
							PageUtility.clickOnElement(moreinfoLink);
							break;
						}
					}
			 }
			// list.add(dashboardIconText);
		}
